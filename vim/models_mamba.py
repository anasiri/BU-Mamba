...
from mamba_ssm.ops.triton.layernorm_gated import RMSNorm, rmsnorm_fn
...
class Block(nn.Module):
    def __init__(
        ...
        if self.fused_add_norm:
            assert RMSNorm is not None, f"{RMSNorm} RMSNorm import fails"
            assert isinstance(
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            fused_add_norm_fn = rmsnorm_fn if isinstance(self.norm, RMSNorm) else layer_norm_fn
            if residual is None:
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.fused_add_norm:
            ...
            fused_add_norm_fn = rmsnorm_fn if isinstance(self.norm_f, RMSNorm) else layer_norm_fn
            hidden_states = fused_add_norm_fn(
                self.drop_path(hidden_states),
                self.norm_f.weight,
                self.norm_f.bias,
